 ...
 *
 * Pure JavaScript over an RGBA array: no DOM, no canvas, no clock, so the same
 * file runs in the PDF tool's page, under vitest in Node, and — later — in the
...
function over(rgba, at, channel) {
  const value = rgba[at + 3];
  if (value === 255) return rgba[at + channel];
  const alpha = value / 255;
  return rgba[at + channel] * alpha + 255 * (1 - alpha);
}
...
function srgbLinear(value) {
  return SRGB_TABLE[clamp(Math.round(value), 0, 255)];
}
...
function lineWindow(values, n, radius, pick, pad, output, line, prefix, suffix) {
  ...
  for (let start = 0; start < length; start += window) {
    ...
    let forward = line[start];
    for (let i = start; i < end; i += 1) {
      forward = pick(forward, line[i]);
      prefix[i] = forward;
    }
    let backward = line[end - 1];
    for (let i = end - 1; i >= start; i -= 1) {
      backward = pick(backward, line[i]);
      suffix[i] = backward;
    }
  }
  for (let i = 0; i < n; i += 1) {
    const at = i + radius;
    output[i] = pick(suffix[at - radius], prefix[at + radius]);
  }
}

/** A separable max or min filter over a whole image. */
function extreme(src, width, height, radius, pick, pad) {
  const data = typeof src.subarray === 'function' ? src : Float32Array.from(src);
  ...
  for (let x = 0; x < width; x += 1) {
    for (let y = 0; y < height; y += 1) values[y] = rows[y * width + x];
    lineWindow(values, height, radius, pick, pad, output, line, prefix, suffix);
    for (let y = 0; y < height; y += 1) out[y * width + x] = output[y];
  }
  return out;
}
...
function sumWindow(input, output, n, radius) {
  ...
  for (let i = 0; i <= first; i += 1) sum += input[i];
  output[0] = sum;
  for (let i = 1; i < n; i += 1) {
    const add = i + radius;
    if (add < n) sum += input[add];
    const drop = i - radius - 1;
    if (drop >= 0) sum -= input[drop];
    output[i] = sum;
  }
}
...
export function boxSum(src, width, height, radius, into) {
  ...
  const r = Math.max(0, Math.round(radius));
  const n = width * height;
  if (r < 1) return into && into.length >= n ? into : Float32Array.from(data);
  const longest = Math.max(width, height);
  ...
  for (let x = 0; x < width; x += 1) {
    for (let y = 0; y < height; y += 1) line[y] = rows[y * width + x];
    sumWindow(line, summed, height, r);
    for (let y = 0; y < height; y += 1) out[y * width + x] = summed[y];
  }
  return out;
}
...
function binaryPass(src, width, height, radius, horizontal, all) {
  ...
  for (let line = 0; line < lines; line += 1) {
    ...
    const first = Math.min(n - 1, radius);
    let ink = 0;
    for (let i = 0; i <= first; i += 1) ink += src[base + i * step] ? 1 : 0;
    for (let i = 0; i < n; i += 1) {
      if (i > 0) {
        const add = i + radius;
        if (add < n) ink += src[base + add * step] ? 1 : 0;
        const drop = i - radius - 1;
        if (drop >= 0) ink -= src[base + drop * step] ? 1 : 0;
      }
...
export function distanceTransform(mask, width, height) {
  ...
  for (let i = 0; i < n; i += 1) d[i] = mask[i] ? far : 0;
  for (let y = 0; y < height; y += 1) {
    for (let x = 0; x < width; x += 1) {
      const i = y * width + x;
      if (d[i] === 0) continue;
      let value = d[i];
      if (x > 0) value = Math.min(value, d[i - 1] + 3);
      if (y > 0) {
        value = Math.min(value, d[i - width] + 3);
        if (x > 0) value = Math.min(value, d[i - width - 1] + 4);
        if (x < width - 1) value = Math.min(value, d[i - width + 1] + 4);
      }
      d[i] = value;
    }
  }
  for (let y = height - 1; y >= 0; y -= 1) {
    for (let x = width - 1; x >= 0; x -= 1) {
      ...
      if (d[i] === 0) continue;
      let value = d[i];
      if (x < width - 1) value = Math.min(value, d[i + 1] + 3);
      if (y < height - 1) {
        value = Math.min(value, d[i + width] + 3);
        if (x < width - 1) value = Math.min(value, d[i + width + 1] + 4);
        if (x > 0) value = Math.min(value, d[i + width - 1] + 4);
...
export function normalise(light, background, width, height) {
  ...
  const out = new Float32Array(n);
  for (let i = 0; i < n; i += 1) {
    out[i] = clamp((light[i] / Math.max(1, background[i])) * 255, 0, 255);
  }
  return out;
}
...
export function sauvola(lightNorm, width, height, k, radius) {
  ...
  for (let i = 0; i < n; i += 1) squares[i] = lightNorm[i] * lightNorm[i];
  const meanSquares = boxMean(squares, width, height, radius);
  const mask = new Uint8Array(n);
  for (let i = 0; i < n; i += 1) {
    const m = mean[i];
    const deviation = Math.sqrt(Math.max(0, meanSquares[i] - m * m));
    const cut = m * (1 + k * (deviation / SAUVOLA_RANGE - 1));
    mask[i] = lightNorm[i] <= cut ? 1 : 0;
  }
  return mask;
}
...
export function chromaInk(channels, width, height, radius, threshold) {
  ...
  if (spreadA * spreadA + spreadB * spreadB <= threshold * threshold) return mask;
  const meanA = boxMean(channels.a, width, height, radius);
  const meanB = boxMean(channels.b, width, height, radius);
  for (let i = 0; i < n; i += 1) {
    const da = channels.a[i] - meanA[i];
    const db = channels.b[i] - meanB[i];
...
export function inkMask(lightNorm, channels, width, height, opts) {
  ...
  const dark = sauvola(
    ...
    width,
    height,
    o.sauvolaK ?? DEFAULT_OPTIONS.sauvolaK,
    windowRadius(o.sauvolaWindow ?? DEFAULT_OPTIONS.sauvolaWindow, unit),
  );
  ...
  for (let i = 0; i < dark.length; i += 1) {
    if (lightNorm[i] <= floor) dark[i] = 1;
  }
  const coloured = chromaInk(
    channels,
    width,
    height,
    windowRadius(o.chromaWindow ?? DEFAULT_OPTIONS.chromaWindow, unit),
    o.chromaThreshold ?? DEFAULT_OPTIONS.chromaThreshold,
  );
  for (let i = 0; i < dark.length; i += 1) {
    if (coloured[i]) dark[i] = 1;
  }
  return dark;
}
...
function ruleLine(mask, width, height, run, thickness, horizontal) {
  ...
  for (let at = 0; at < n; at += 1) {
    if (mask[at]) counts[horizontal ? (at / width) | 0 : at % width] += 1;
  }
  let most = 0;
  for (let k = 0; k < outer; k += 1) {
    if (counts[k] > most) most = counts[k];
  }
  if (most < run) return null;
  ...
  for (let line = 0; line < outer; line += 1) {
    ...
    for (let i = 0; i < inner; i += 1) {
      ...
      for (let k = 1; k <= thickness; k += 1) {
        if (acrossAt - k < 0 || !mask[at - k * across]) break;
        thick += 1;
      }
      for (let k = 1; k <= thickness; k += 1) {
        if (acrossAt + k >= acrossLimit || !mask[at + k * across]) break;
        thick += 1;
      }
      if (thick > thickness) continue;
      for (let k = -thickness; k <= thickness; k += 1) {
        const away = acrossAt + k;
        if (away < 0 || away >= acrossLimit) continue;
        const to = at + k * across;
        if (mask[to]) flags[to] = 1;
      }
    }
  }
  return flags;
...
export function removeRules(mask, width, height, opts) {
  ...
  const thickness = Math.max(
    3,
    Math.round((o.ruleThickness ?? DEFAULT_OPTIONS.ruleThickness) * unit),
  );
  const n = width * height;
  const ink = Uint8Array.from(mask);
  const across = Math.round(span * width);
  const horizontal =
    across >= MIN_RULE_KERNEL && across < width
      ? ruleLine(mask, width, height, across, thickness, true)
      : null;
  const down = Math.round(span * height);
  const vertical =
    down >= MIN_RULE_KERNEL && down < height
      ? ruleLine(mask, width, height, down, thickness, false)
      : null;
  if (!horizontal && !vertical) return ink;
  for (let at = 0; at < n; at += 1) {
    if ((horizontal && horizontal[at]) || (vertical && vertical[at])) ink[at] = 0;
  }
...
function marksFromMask(mask, width, height, minArea, distance) {
  ...
  for (let start = 0; start < n; start += 1) {
    ...
    for (let k = 0; k < count; k += 1) widths[k] = 2 * distance[pixels[k]];
    ...
    marks.push({
      ...
      box,
      area: count,
      strokeWidth: sorted[sorted.length >> 1],
...
export function strokeLike(mark, limits) {
  ...
  if (mark.strokeWidth < minStroke || mark.strokeWidth > maxStroke) return false;
  if (mark.elongation < (o.minElongation ?? DEFAULT_OPTIONS.minElongation)) return false;
...
function sameLine(a, b) {
  ...
  if (taller > shorter * 1.8) return false;
  const bottomA = a.box.y + a.box.h;
  const bottomB = b.box.y + b.box.h;
  if (Math.abs(bottomA - bottomB) > 0.4 * shorter) return false;
...
export function textRow(marks, width, height) {
  ...
  const groups = [];
  const order = [];
  for (let i = 0; i < marks.length; i += 1) order.push(i);
  order.sort((a, b) => marks[a].box.y - marks[b].box.y);
  for (let k = 0; k < order.length; k += 1) {
    const i = order[k];
    if (marks[i].box.h > 0.5 * height) continue;
    let group = null;
    for (let g = 0; g < groups.length; g += 1) {
      if (sameLine(marks[groups[g][0]], marks[i])) {
        group = groups[g];
        break;
      }
    }
  ...
  for (let g = 0; g < groups.length; g += 1) {
    ...
    for (let k = 0; k < groups[g].length; k += 1) flags[groups[g][k]] = 1;
  }
  return flags;
}
...
export function chooseWriting(marks, width, height, opts) {
  ...
  for (let i = 0; i < marks.length; i += 1) {
    stroke[i] = strokeLike(marks[i], limits) ? 1 : 0;
    if (stroke[i] && !text[i]) seeds.push(i);
  }
  if (seeds.length === 0) {
    ...
    for (let i = 0; i < marks.length; i += 1) {
      if (text[i]) continue;
      const score = penScore(marks[i], limits);
  ...
  const take = (i) => {
    ...
    for (let k = 0; k < pixels.length; k += 1) chosen[pixels[k]] = 1;
    taken[i] = 1;
  };
  for (let k = 0; k < seeds.length; k += 1) take(seeds[k]);
  const gap = Math.max(
    1,
    Math.round((o.joinGap ?? DEFAULT_OPTIONS.joinGap) * Math.max(width, height)),
  );
  ...
  while (grew && rounds < 16) {
    ...
    for (let i = 0; i < marks.length; i += 1) {
      if (taken[i]) continue;
      if (text[i] && !touches(marks[i], tightNear)) continue;
      if (!touches(marks[i], near)) continue;
      take(i);
      grew = true;
    }
  }
  ...
  for (let i = 0; i < marks.length; i += 1) {
    if (!taken[i]) continue;
    selectedArea += marks[i].area;
    if (stroke[i]) strokeArea += marks[i].area;
  }
...
export function softAlpha(light, background, chosen, width, height, opts) {
  ...
  for (let i = 0; i < n; i += 1) {
    if (chosen[i] && light[i] < inkLight) inkLight = light[i];
  }
  if (inkLight === Infinity) return alpha;
  const grow = Math.max(0, Math.round(o.grow ?? DEFAULT_OPTIONS.grow));
  const wide = grow > 0 ? dilate(chosen, width, height, grow) : chosen;
  for (let i = 0; i < n; i += 1) {
    if (!wide[i]) continue;
    const paper = Math.max(inkLight + 1, background[i]);
    alpha[i] = clamp((paper - light[i]) / (paper - inkLight), 0, 1);
  }
  return alpha;
}
...
export function paperColour(rgba, mask, width, height) {
  ...
  const out = { r: r / weight, g: g / weight, b: b / weight };
  out.light = Math.max(1, RED * out.r + GREEN * out.g + BLUE * out.b);
  return out;
}

/** What a repainted pen is, or null when the pen keeps its own colour. */
function flatColour(mode) {
  if (mode === 'black') return hexToRgb(INK_BLACK);
  if (mode === 'blue') return hexToRgb(INK_BLUE);
  return null;
}
...
export function inkColours(rgba, alpha, background, width, height, paper, mode) {
  ...
  for (let i = 0; i < n; i += 1) {
    ...
    const a = Math.max(alpha[i], 0.2);
    const scale = clamp(background[i] / base.light, 0.2, 4);
...
export function dominantInkColour(colours, alpha, width, height) {
  ...
  let most = 0;
  for (let i = 0; i < n; i += 1) {
    if (alpha[i] > most) most = alpha[i];
  }
  if (most <= 0) return INK_BLACK;
  ...
  let count = 0;
  for (let i = 0; i < n; i += 1) {
    if (alpha[i] < floor) continue;
    count += 1;
    const at = i * 3;
    red[colours[at]] += 1;
    green[colours[at + 1]] += 1;
    blue[colours[at + 2]] += 1;
  }
  if (count === 0) return INK_BLACK;
...
export function inkBounds(alpha, width, height, padding) {
  ...
  const pad = Math.round(
    (padding ?? DEFAULT_OPTIONS.padding) * Math.max(maxX - minX + 1, maxY - minY + 1),
  );
...
export function cropRgba(rgba, width, height, bounds) {
  ...
  for (let y = 0; y < bounds.h; y += 1) {
    ...
    for (let k = 0; k < bounds.w * 4; k += 1) out[to + k] = rgba[from + k] ?? 0;
  }
  return out;
}
...
export function fitToMaxSide(rgba, width, height, maxSide) {
  ...
  if (Math.max(width, height) <= cap) return { rgba, width, height };
...
function readInk(pixels, width, height, opts) {
  ...
  const radius = Math.max(
    2,
    Math.round(
      (opts.backgroundRadius ?? DEFAULT_OPTIONS.backgroundRadius) * Math.min(width, height),
    ),
  );
...
export function liftSignature(rgba, width, height, opts) {
  ...
  const marks = components(read.mask, w, h, { minArea: o.minArea, distance });
  if (marks.length === 0) return null;
  const writing = chooseWriting(marks, w, h, o);
  if (writing.selectedArea === 0) return null;
  ...
  for (let y = 0; y < bounds.h; y += 1) {
    for (let x = 0; x < bounds.w; x += 1) {
      ...
      cut[to] = colours[from * 3];
      cut[to + 1] = colours[from * 3 + 1];
      cut[to + 2] = colours[from * 3 + 2];
      cut[to + 3] = alpha[from] * 255;
    }
  }
  let inkArea = 0;
  for (let i = 0; i < w * h; i += 1) {
    if (read.mask[i]) inkArea += 1;
  }
